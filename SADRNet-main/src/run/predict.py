...
class Evaluator:
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.all_eval_data)):
                ...
                if is_visualize:
                    ...
                    print(f'{output_folder}/{i}_kpt.jpg', ret_kpt)
    ...
    def evaluate_example_one_image(self, predictor, input_image='data/example/1.jpg',init_img=None, is_visualize=True, output_folder='data/output/SADRN-out'):
        with torch.no_grad():
            predictor.model.eval()
            if init_img is None:
                init_img = np.array(Image.open(input_image))
            image = (init_img / 255.0).astype(np.float32)
            for ii in range(3):
                image[:, :, ii] = (image[:, :, ii] - image[:, :, ii].mean()) / np.sqrt(
                    image[:, :, ii].var() + 0.001)
            image = img_to_tensor(image).to(config.DEVICE).float().unsqueeze(0)
            out = predictor.model({'img': image}, 'predict')
            # print(f'{input_image}/{len(self.all_eval_data)}')

            if is_visualize:
                face_uvm_out = out['face_uvm'][0].cpu().permute(1, 2, 0).numpy() * config.POSMAP_FIX_RATE
                ret, ret_kpt = self.show_face_uvm(face_uvm_out, init_img, None, True)
                # io.imsave(f'{output_folder}/{i}_cmp.jpg', ret_cmp)
                io.imsave(f'{output_folder}/{input_image}_kpt.jpg', ret_kpt)
                io.imsave(f'{output_folder}/{input_image}_face.jpg', ret)
                io.imsave(f'{output_folder}/{input_image}_img.jpg', init_img)
            else:
                face_uvm_out = out['face_uvm'][0].cpu().permute(1, 2, 0).numpy() * config.POSMAP_FIX_RATE
                ret, ret_kpt = self.show_face_uvm(face_uvm_out, init_img, None, False)
                return ret_kpt
...
if __name__ == '__main__':
    ...
    if config.NET == 'SADRNv2':
        ...
        evaluator.evaluate_example_one_image(predictor_1)
